<template>
  <q-page class="max-w-1200 m-auto xs:p-15 sm:p-0 xs:pb-20 sm:pb-40">
    <div class="upperSlot flex justify-between items-center xs:mt-0 xs:mb-20 sm:mb-0 sm:mt-30">
      <div>
        <q-icon class="xs:text-14 sm:text-24 text-tw-grey-darkest xs:mr-10 sm:mr-20 align-middle" name="icon-data" />
        <span class="xs:text-14 sm:text-24 font-semibold text-tw-grey-darkest  align-middle">{{$t('DIGITAL_ASSET')}}</span>
      </div>
    ...
    <table-container class="custorm-thead" :data="data" :count="count" :params="params" :columnsData="columnsData" @getData="getData">
      <template class="desktop-only" slot="content" slot-scope="props" v-if="props.props">
        <q-td v-if="props.props.name" key="asset">
          <div class="text-tw-blue cursor-pointer hover:underline" @click="doSearch(props.props.name, 'asset')">
